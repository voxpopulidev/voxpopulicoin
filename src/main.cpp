...
int nCoinbaseMaturity = 40; //20 blocks for coins to mature
...
const string strMessageMagic = "voxpopuli Signed Message:\n";
...
int64_t GetProofOfWorkReward(int64_t nFees)
{
	...
	if(pindexBest->nHeight == 1)
        ...
        if(pindexBest->nHeight <= 5760)
        {
         nSubsidy = 0 * COIN;
         return nSubsidy + nFees;
        }
        if(pindexBest->nHeight <=12960)
        {
         nSubsidy = 750 * COIN;
          return nSubsidy + nFees;
        }
...
bool CBlock::AcceptBlock()
{
    ...
    if (IsProofOfStake())
    {
        ...
        {
            printf("WVOXPING: ProcessBlock(): check proof-of-stake failed for block %s\n", hash.ToString().c_str());
    ...
    if (CheckpointsMode == Checkpoints::ADVISORY && !cpSatisfies)
        strMiscWarning = _("WVOXPING: syncronized checkpoint violation detected, but skipped!");
...
bool CheckDiskSpace(uint64_t nAdditionalBytes)
{
    ...
    if (nFreeBytesAvailable < nMinDiskSpace + nAdditionalBytes)
    {
        ...
        printf("*** %s\n", strMessage.c_str());
        uiInterface.ThreadSafeMessageBox(strMessage, "voxpopuli", CClientUIInterface::OK | CClientUIInterface::ICON_EXCLAMATION | CClientUIInterface::MODAL);
        StartShutdown();
        return false;
    }
    return true;
...
bool LoadBlockIndex(bool fAllowNew)
{
    ...
    if (mapBlockIndex.empty())
    {
        ...
        const char* pszTimestamp = "voxpopuli10/02";
        CTransaction txNew;
        txNew.nTime = 1412261749;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        ...
        //// debug print
        assert(block.hashMerkleRoot == uint256("0x86fbba2ca7d2b89a73a6324e7fce31069c48ee7a55117b149fdbfa499f4d07ee"));
        block.print();
...
string GetWarnings(string strFor)
{
    ...
    {
        nPriority = 3000;
        strStatusBar = strRPC = _("WVOXPING: Invalid checkpoint found! Displayed transactions may not be correct! You may need to upgrade, or notify developers.");
    }
...
unsigned char pchMessageStart[4] = { 0xcd, 0xdc, 0xef, 0x1e };
